...
export function stringsToIntegers(numbers: string[]): number[] {
    return numbers.map((num: string): number => {
        let someNum: number = parseInt(num);
        if (Number.isNaN(someNum)) return 0;
        return someNum;
    });
}
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    let newStrings: string[] = messages.filter(
        (str: string): boolean => str[str.length - 1] !== "?",
    );
    return newStrings.map((str: string): string =>
...
export function makeMath(addends: number[]): string {
    ...
    let totalString: string = addends.reduce(
        (currentString: string, num: number, index: number) =>
            currentString +
            num.toString() +
